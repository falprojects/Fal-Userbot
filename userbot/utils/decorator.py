    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            fal_reg = sudo_reg = re.compile(pattern)
        else:
            fal_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            fal_reg = re.compile(fal_ + pattern)
        ...
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(
                    **args, outgoing=True, pattern=fal_reg))
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=fal_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
